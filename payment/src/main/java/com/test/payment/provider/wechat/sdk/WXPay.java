package com.test.payment.provider.wechat.sdk;

import com.test.payment.provider.wechat.sdk.WXPayConstants.SignType;
...
public class WXPay {
    ...
    private Map<String, String> requestSandboxKey(String key) throws Exception {
        ...
        if (respData.containsKey(returnCode)) {
            returnCode = respData.get(returnCode);
    ...
    private void checkWXPayConfig() {
        ...
        if (this.config.getAppID() == null || this.config.getAppID().trim().length() == 0) {
            throw new RuntimeException("appid in config is empty");
        }
        ...
        if (this.config.getHttpConnectTimeoutMs() < 10) {
            throw new RuntimeException("http connect timeout is too small");
        }
        if (this.config.getHttpReadTimeoutMs() < 10) {
            throw new RuntimeException("http read timeout is too small");
        }
