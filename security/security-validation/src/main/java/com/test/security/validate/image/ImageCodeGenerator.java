...
public class ImageCodeGenerator implements ValidateCodeGenerator {
    ...
    public ImageCodeGenerator(ValidationProperties validationProperties) {
	...
	public ImageCode generate(ServletWebRequest request) {
		int width = ServletRequestUtils.getIntParameter(request.getRequest(), "width",
                validationProperties.getImage().getWidth());
		int height = ServletRequestUtils.getIntParameter(request.getRequest(), "height",
                validationProperties.getImage().getHeight());
		...
		return new ImageCode(image, randomString, validationProperties.getImage().getExpireIn());
	}
	...
	private String getRandomString() {
        String str;
        do {
            str = RandomStringUtils.randomAlphanumeric(validationProperties.getImage().getLength());
            // 排除I l 1 这种模糊不清的
